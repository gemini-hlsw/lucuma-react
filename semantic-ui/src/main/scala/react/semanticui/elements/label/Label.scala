...
object Label {
  ...
  def props(
    ...
    q.pointing.toJs.foreachUnchecked(v => p.pointing = v)
    q.prompt.foreach(v => p.prompt = v)
    CompToPropsS(q.removeIcon).toJs.foreachUnchecked(v => p.removeIcon = v)
    q.ribbon.toJs.foreachUnchecked(v => p.ribbon = v)
