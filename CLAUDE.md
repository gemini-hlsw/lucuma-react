...
The build targets **JDK 25** (enforced by sbt-typelevel — an older JDK fails with "Target JDK is 25
but you are using an older JDK"). Make sure `java` on your `PATH` is JDK 25+. A Nix devshell that
pins the toolchain is available (`nix develop`, configured in `flake.nix`'s `jdk.package`) but is
optional. If sbt picks up the wrong JDK despite a correct `JAVA_HOME`, a stale sbt/BSP server (often
kept alive by an IDE/Metals) is usually the culprit — `sbt shutdown` and retry.
...
sbt gridLayoutDemo/fastLinkJS && npx vite -c grid-layout-demo/vite.config.js

# Regenerate a demo's vite config (goes stale on every Scala 3 patch release)
sbt gridLayoutDemo/viteConfigGenerate
```

## Key Versions

- Scala 3.8.4 · scalajs-react 4.0.0 · cats 2.13.0 · lucuma-typed 0.12.0
