...
case class VirtualOptions[TScrollElement <: Element, TItemElement <: Element](
   ...
   * their height.
   */
  scrollMargin:          js.UndefOr[SizePx] = js.undefined,
  /**
   ...
   * scrollEnd event.
   */
  isScrollingResetDelay: js.UndefOr[Int] = js.undefined,
  /**
   ...
   * scrollEnd event.
   */
  useScrollendEvent:     js.UndefOr[Boolean] = js.undefined,
  ...
  def toJs: VirtualOptionsJS[TScrollElement, TItemElement] = {
    ...
    scrollMargin.foreach(v => p.scrollMargin = v.value)
    gap.foreach(v => p.gap = v.value)
    lanes.foreach(v => p.lanes = v)
    isScrollingResetDelay.foreach(v => p.isScrollingResetDelay = v)
    useScrollendEvent.foreach(v => p.useScrollendEvent = v)
    isRtl.foreach(v => p.isRtl = v)
    p
  }
