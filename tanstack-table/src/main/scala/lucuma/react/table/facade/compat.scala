...
/**
 * Compatibility shim that re-exports the TanStack Table **v9** generated types under the
 * v8-style `buildLib*Mod` module names that the hand-maintained facade was written against.
 *
 * v9 reorganised `@tanstack/table-core`'s source tree under `dist/`, so ScalablyTyped now emits
 * modules named after those paths (e.g. `distTypesTableMod`) instead of the old
 * `buildLibTypesMod`. Rather than rewrite every `raw.buildLibFooMod.X` reference across ~30 files,
 * this object provides stable short aliases. All v9 core types gained a leading `TFeatures` type
 * parameter; we pin it to `LegacyFeatures` (the full feature set registered by `useLegacyTable`),
 * which restores the v8 single-generic `[TData]` shape the facade assumes.
 *
...
object compat:
  ...
  object buildLibCoreHeadersMod:
    type HeaderContext[T, V] =
      lucuma.typed.tanstackTableCore.distCoreHeadersCoreHeadersFeatureDottypesMod.HeaderContext[LF, T, V]
