...
object Virtuoso {
  ...
  // Eliminate some type ascriptions below. Incidentally, it also catches errors when
  ...
  implicit def FuncOps1[A, B](f:    A => B): js.UndefOr[js.Function1[A, B]]         = f: js.Function1[A, B]
  implicit def FuncOps2[A, B, C](f: (A, B) => C): js.UndefOr[js.Function2[A, B, C]] =
